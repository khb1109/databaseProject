...
public class buy extends JFrame {
	...
	public buy(int n) // n은  item의 시리얼 값
	{
	...
	class BuyActionListener implements ActionListener{
		...
		{// 추가할때 조건들을 잘 걸어야함.
			...
			int cnt = Integer.parseInt(tf1.getText()); // 재고수 입력한걸 인티저 형으로 변환
			...
			if(!str[1].equals(tf3.getText())) // 고객번호로 얻은 이름과 입력한 이름이 같아야함.
				return;
			...
			try{
				con.rs = con.st.executeQuery(sql);
				con.rs.next();
				e_name = con.rs.getString(2); // 처리할 직원
				
				sql = "insert into infoOrder(e_Name,c_Name,i_name,cnt,price,paymentDay,"
						+ "expectationDay,delivery_date)value('"+e_name+"','"+c_name+"','"+i_name+"','"+cnt+"','"+price+"','"+day+"','"+"미정"+"','"+"미정"+"');";	
			...
			JOptionPane.showMessageDialog(null, "구매완료, 감사합니다!");
